...
class _TestAuthorization(_Unittest):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.host = cls.server.config['hostname']
        ...
        cls.not_daily_gmpasses = {cls.server.config['gmpass{}'.format(((current_day + x)% 7) + 1)]
                                  for x in range(1, 7)}
...
class _TestAuthorizationSingleRank(_TestAuthorization):
    ...
    def test_03_RightLoginAndRelogin(self):
        ...
        self.c0.assert_received_ooc(self.host, self.ooc_successful_login, over=True)
    ...
    def test_06_TwoSimultaneousSameRank(self):
        ...
        for [pass1, pass2] in pass_pos:
            for (a1, a2, b1, b2) in client_pos:
                ...
                a1.assert_received_ooc(self.host, self.ooc_successful_login, over=True)
                ...
                a2.assert_received_ooc(self.host, self.ooc_successful_login, over=True)
                self.assertTrue(self.good_rank(a1))
...
class Test01_AuthorizationGM(_TestAuthorizationSingleRank):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
...
class Test02_AuthorizationCM(_TestAuthorizationSingleRank):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
...
class Test03_AuthorizationMod(_TestAuthorizationSingleRank):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
...
class Test04_AuthorizationIntegration(_TestAuthorization):
    def test_02_WrongLogins(self):
        ...
        self.c1.assert_received_ooc(self.host, 'Invalid password.', over=True)
        ...
        self.c2.assert_received_ooc(self.host, 'Invalid password.', over=True)
    ...
    def test_03_RightLoginAndRelogin(self):
        ...
        self.c1.assert_received_ooc(self.host, 'Logged in as a community manager.', over=True)
        ...
        self.c2.assert_received_ooc(self.host, 'Logged in as a moderator.', over=True)
        self.assertTrue(self.c2.is_mod)

        self.c2.ooc('/login {}'.format(self.modpass))
        self.c2.assert_received_ooc(self.host, 'Already logged in.', over=True)
        self.assertTrue(self.c2.is_mod)

        self.c1.ooc('/logincm {}'.format(self.cmpass))
        self.c1.assert_received_ooc(self.host, 'Already logged in.', over=True)
        self.assertTrue(self.c1.is_cm)
    ...
    def test_04_LogoutAndRelogout(self):
        ...
        self.c2.assert_received_ooc(self.host, 'This command has no arguments.', over=True)
        self.assertFalse(self.c0.is_gm)
        self.assertTrue(self.c1.is_cm)
        self.assertTrue(self.c2.is_mod)

        self.c1.ooc('/logout')
        self.c1.assert_received_ooc(self.host, 'You are no longer logged in.', ooc_over=True)
        ...
        self.c2.ooc('/logout')
        self.c2.assert_received_ooc(self.host, 'You are no longer logged in.', ooc_over=True)
        ...
        self.c1.assert_received_ooc(self.host, 'This command has no arguments.', over=True)
        self.assertFalse(self.c0.is_gm)
        self.assertFalse(self.c1.is_cm)
        self.assertFalse(self.c2.is_mod)
    ...
    def test_05_LoginLogout(self):
        ...
        def login(c1, c2, ooc_rank1, ooc_rank2, pass1, pass2, rank1, rank2, success1, success2):
            ...
            c1.assert_received_ooc(self.host, success1, over=True)
            self.assertTrue(rank1(c1))

            c1.ooc('/logout')
            c1.assert_received_ooc(self.host, 'You are no longer logged in.', ooc_over=True)
            ...
            c2.assert_received_ooc(self.host, success2, over=True)
            self.assertTrue(rank2(c2))

            c2.ooc('/logout')
            c2.assert_received_ooc(self.host, 'You are no longer logged in.', ooc_over=True)
    ...
    def test_06_OneSingleRankPerPlayer(self):
        ...
        self.c0.assert_received_packet('FM', None)
        self.c0.assert_received_ooc(self.host, 'Logged in as a community manager.', over=True)
        self.assertFalse(self.c0.is_mod)
        ...
        self.c0.assert_received_packet('FM', None)
        self.c0.assert_received_ooc(self.host, 'Logged in as a moderator.', over=True)
...
class Test05_AuthorizationShortcuts(_TestAuthorization):
    """
