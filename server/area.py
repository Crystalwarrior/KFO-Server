...
class Area:
    ...
    def __init__(self, area_manager: "AreaManager", name: str):
        ...
        self.timers: List[Timer] = [Timer(x) for x in range(20)]
    ...
    def send_ic(
        ...
        if client and msg.startswith("++") and len(self.testimony) > 0:
            ...
            if msg.strip() != "" or self.last_ic_message is None or pos != self.last_ic_message[8] or self.last_ic_message[4].strip() != "":
    ...
    def add_jukebox_vote(self, client, music_name, length=-1, showname=""):
        ...
        if client.change_music_cd():
            client.send_ooc(f"You changed the song too many times. Please try again after {int(client.change_music_cd())} seconds.")
            return
        if length == 0:
    ...
    def start_debate(self, client, target, pta=False):
        ...
        self.minigame_schedule = asyncio.get_running_loop().call_later(timer, lambda: self.end_minigame("Timer expired!"))
