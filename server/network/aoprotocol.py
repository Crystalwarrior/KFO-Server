...
from server.constants import censor, contains_URL, derelative, dezalgo
...
class AOProtocol(asyncio.Protocol):
    ...
    def net_cmd_ms(self, args):
        ...
        if self.validate_net_cmd(
            ...
            try:
                ...
                self.client.send_ooc("Something went wrong! Please report the issue to the developers.")
                return
        elif self.validate_net_cmd(
            args,
            self.ArgType.STR,  # 0 # msg_type
        ...
        if self.client.area.cannot_ic_interact(self.client, button):
            ...
            return False
        if button == "0" and not self.client.area.can_send_message():
            return
        ...
        if self.client.area.is_iniswap(self.client, pre, anim, folder, sfx):
            ...
            self.client.send_ooc(f"Iniswap/custom emotes are blocked in this area for character '{folder}', pre '{pre}' anim '{anim}'.")
            return
        ...
        if (nonint_pre == 1 and button in range(1, 4)) or (
            ...
            if emote_mod == 1 or emote_mod == 2:
                ...
                emote_mod = 5
                nonint_pre = 1
        if not self.client.area.shouts_allowed and not self.client.is_mod and self.client not in self.client.area.owners:
        ...
        if text.lower().lstrip().startswith("/w"):
            if not self.client.area.can_whisper and not self.client.is_mod and self.client not in self.client.area.owners:
            ...
            try:
                ...
                if len(clients) > 0:
                    part = part[1:]
                    whisper_clients = [c for c in self.client.area.clients if str(c.id) in clients and not c == self.client]
                    clients = ",".join(clients)
                else:
                    whisper_clients = [c for c in self.client.area.clients if c.pos == self.client.pos and not c == self.client]
                    clients = ""
                text = " ".join(part)
                text = "}}}[W" + clients + "] {{{" + text
            except (ValueError, AreaError):
        ...
        if evidence:
            ...
            try:
                ...
                self.client.area.broadcast_ooc(f"[{self.client.id}] {self.client.showname} has presented evidence: {evi.name}.")
        ...
        if len(target_area) > 0:
            try:
                for a in target_area:
                    ...
                    if len(a.pos_lock) > 0:
                        tempos = a.pos_lock[0]
                    if a.last_ic_message is not None and (anim == "" or len(a.pos_lock) <= 0 or a.last_ic_message[5] not in a.pos_lock):
                        # Use the same pos
        ...
        if whisper_clients is None:
            ...
            if text.strip() != "" or self.client.area.last_ic_message is None or self.client.area.last_ic_message[4].strip() != "":
                # Discord Bridgebot
                if (
        ...
        if "¨" in text:
            ...
            for index, message in enumerate(messages):
                ...
                if stripped_message.startswith("<") and stripped_message.endswith(">"):
                    ...
                    emote_value = anim if index == 0 else emote  # Use 'anim' if it's the first message, otherwise use the emote variable
                    additive_value = 0 if index == 0 else 1  # Set additive_value to 0 for the first message, 1 for subsequent messages
    ...
    def net_cmd_ct(self, args):
        ...
        if self.client.ooc_mute():
            self.client.send_ooc(f"You are using OOC too fast. Please try again after {int(self.client.ooc_mute())} seconds.")
            return
        for c in args[0]:
            if unicodedata.category(c) == "Cf":
                self.client.send_ooc("You cannot use format characters in your name!")
                return
        if args[0].startswith(self.server.config["hostname"]) or args[0].startswith("<dollar>G") or args[0].startswith("<dollar>M"):
        ...
        if self.client.area.area_manager.censor_ooc and self.server.censors is not None and len(self.server.censors) > 0:
            # Censor the name
            args[0] = censor(
                args[0],
                self.server.censors["whole"],
    ...
    def net_cmd_casea(self, args):
        ...
        if self.client in self.client.area.owners:
            ...
            if not args[1] == "1" and not args[2] == "1" and not args[3] == "1" and not args[4] == "1" and not args[5] == "1":
                ...
                return
            msg = "=== Case Announcement ===\r\n{} [{}] is hosting {}, looking for ".format(self.client.showname, self.client.id, args[0])
    ...
    def net_cmd_hp(self, args):
        ...
        if self.client.is_muted:  # Checks to see if the client has been muted by a mod
            ...
            return
        if self.client.area.cannot_ic_interact(self.client):
            self.client.send_ooc("You are not on the area's invite list, and thus, you cannot change the Confidence bars!")
            return
