    ...
    def data_received(self, data):
        ...
        for msg in self.get_messages():
            ...
            try:
                ...
                logger.error("Error handling message from %s: %s", ipid, msg, exc_info=True)
    ...
    def net_cmd_ct(self, args):
        ...
        if (
            ...
            if "ooc_chat" in self.server.config["bridgebot"] and self.server.config["bridgebot"]["ooc_chat"]:
                self.server.bridgebot.queue_message(webname, args[1])
    ...
    def net_cmd_cu(self, args):
        ...
        if old_link != self.client.char_url:
            ...
            if self.client.char_url == "":
                self.client.area.broadcast_ooc(
                    f"[{self.client.id}] {self.client.showname} has cleared their download link."
                )
            else:
                self.client.area.broadcast_ooc(
                    f"[{self.client.id}] {self.client.showname} has set their download link to:\n{self.client.char_url}"
                )
