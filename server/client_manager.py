    ...
    class Client:
        ...
        def kick_to_latest_area(self):
            ...
            if target_area is None:
                self.send_ooc(f"Attempted to kick you to area ID {latest_area} but that area does not currently exist!")
                return
            old_area = self.area
        ...
        def change_character(self, char_id, force=False):
            ...
            if self.first_joined:
                ...
                self.area.broadcast_ooc(f"[{self.id}] {self.showname} has joined!")
                ...
                if not old_area.dark and not old_area.force_sneak:
                    if old_area.area_manager == self.area.area_manager:
                        ...
                        old_area.broadcast_ooc(
                            f"[{self.id}] {self.showname} leaves to [{self.area.id}] {self.area.name}.", exclude_list
                        )
                    else:
                        old_area.broadcast_ooc(
        ...
        def update_evidence_list(self):
            ...
            if self.blinded:
                evi_list.insert(
                    ...
                    (
                        "Blinded!",
                        "You are blind!\n\nYou are unable to see any IC messages or edit any evidence at this time.",
                        "BLIND\n🕶️",
                    ),
                )
            ...
            if self.viewing_inventory:
                evi_list.insert(
                    ...
                    (
                        "Inventory",
                        "Delete this piece of evidence to swap to 🌐Area Evidence!\n\nYou can 'present' things in your inventory to drop it into the Area Evidence.\n\n⚠️Deleting evidence in your Inventory will permanently get rid of it!",
                        "INV.\n🎒",
                    ),
                )
            else:
                evi_list.insert(
                    ...
                    (
                        "Area",
                        "Delete this piece of evidence to swap to 🎒Inventory Evidence!\n\nIf you want to take things in the area into your evidence, you need to press 'Delete' for that evidence as well.",
                        "AREA\n🌐",
                    ),
                )
            self.send_command("LE", *evi_list)
