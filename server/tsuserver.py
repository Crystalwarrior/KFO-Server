...
from server.constants import dezalgo, remove_URL
...
class TsuServer:
    ...
    def new_client(self, transport: asyncio.BaseTransport) -> "Client":
        ...
        for line, rangeBan in enumerate(self.ipRange_bans):
            if rangeBan != "" and (
                (peername.startswith(rangeBan) and (rangeBan.endswith(".") or rangeBan.endswith(":"))) or asn == rangeBan
            ):
                msg = "BD#"
                msg += "Abuse\r\n"
                msg += f"ID: {line}\r\n"
    ...
    def refresh(self) -> None:
        ...
        with open("config/config.yaml", "r") as cfg:
            ...
            for profile in self.config["modpass"]:
                if profile not in cfg_yaml["modpass"] or self.config["modpass"][profile] != cfg_yaml["modpass"][profile]:
