...
* Deprecation warnings are no sent for uses of the optional parameter `ic_params` (a list of IC argument values) in ClientManager.Client.send_ic and ClientManager.Client.send_ic_others. The recommended parameter is now `params` (a dictionary matching argumet names to their values).
